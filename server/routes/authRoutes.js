...
router.post('/signup', async (req, res) => {
    ...
    if (role !== "expertise" && role !== "vet") {
        if (!req.body.password || !req.body.confirmpassword) {
            return res.status(422).send({ error: "Password and confirm password are required" });
        }
...
router.post('/approve-vet', async (req, res) => {
    ...
    try {
        ...
        if (isApproved) {
            ...
            return res.send({ message: "Vet approved successfully and login credentials sent." });
        } else {
            ...
            return res.send({ message: "Vet registration rejected and user removed." });
        }
    } catch (error) {
...
router.post('/login', async (req, res) => {
    ...
    try {
        const user = await User.findOne({ email: emailTrimmed }).populate('role_id');
        console.log(user)
        ...
        if (user.role_id.role_type === "vet" && !user.is_approved) {
            return res.status(403).json({ error: "Vet registration is pending approval" });
        }

        const token = jwt.sign({ userId: user._id, role: user.role_id.role_type }, process.env.JWT_SECRET, { expiresIn: "1d" });
