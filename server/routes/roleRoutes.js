...
router.post('/roles', async (req, res) => {
    ...
    if (!role_id || !role_type || !role_description) {
        return res.status(400).json({ error: "role_id, role_type, and role_description are required" });
    }
...
router.put('/roles/:id', async (req, res) => {
    ...
    if (!role_type && !role_description) {
        return res.status(400).json({ error: "At least one field (role_type or role_description) is required" });
    }

    try {
        const updatedRole = await Role.findByIdAndUpdate(req.params.id, { role_type, role_description }, { new: true });
